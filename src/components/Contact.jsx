      ...
      <div className="w-full flex flex-col specific:flex-row specific:justify-between justify-center specific:align-middle">
        ...
        <div className="flex flex-col w-fit mx-auto justify-center ">
          <form onSubmit={_submit} className="relative">
            ...
            <div className={`bg-card mb-[.6rem] pt-[.35rem] pb-2 px-3 mx-auto w-[32ch] rounded-[.3rem] text-text font-sans font-light text-[1.1rem] focus-within:ring-1 transition-all duration-300 ease-in-out focus-within:ring-accent ${!send ? "opacity-1000" : "opacity-0"}`}>
              <input value={topic} type="title" required placeholder="title" onChange={(e) => setTopic(e.target.value)} className="rounded-sm w-full outline-none  autofill:bg-card bg-transparent text-text font-sans font-light text-[1.1rem] " />
            </div>
            <div className={`bg-card mb-[.6rem] pt-[.35rem] pb-2 px-3 mx-auto w-[32ch] rounded-[.3rem] text-text font-sans font-light text-[1.1rem] focus-within:ring-1 transition-all duration-300 ease-in-out focus-within:ring-accent ${!send ? "opacity-1000" : "opacity-0"}`}>
              <textarea value={message} type="text" required placeholder="message" onChange={(e) => setMessage(e.target.value)} className="w-full autofill:bg-card max-w-full h-32 min-h-[8rem] max-h-48 overflow-x-hidden overflow-y-auto outline-none bg-transparent text-text font-sans font-light text-[1.1rem] " />
            </div>
            ...
            <div className={`absolute flex flex-col justify-center align-middle inset-0 rounded-[.3rem] transition-all duration-300 ease-in-out bg-card ${send ? "scale-100 opacity-1000" : "scale-0 opacity-0"}`}>
              ...
              <button className="w-10/12 mx-auto outline-none autofill:bg-card bg-transparent  text-accent font-sans font-thin text-base" onClick={() => setSend(false)}>Send another message</button>
            </div>
          </form>
        </div>
      </div>
